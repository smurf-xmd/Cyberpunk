...
function logBox(sender, msgType, result) {
    ...
    console.log(`${G}${B} STATUS${X} ${C}${sender}${X} ${D}${msgType}${X} ${G}${result}${X} ${D}${t}${X}`);
}
...
addCmd({
    ...
    desc: 'Automatically view (mark as seen) WhatsApp statuses ',
    category: 'automation',
    ownerOnly: true,
    handler: async (ctx) => {
        try {
            ...
            switch (action) {
                case 'on': case 'enable': case 'start': {
                    ...
                    await reply(`*AUTOVIEWSTATUS ENABLED*\n\nWill now automatically view statuses!`);
                    break;
                }
                ...
                case 'include': case 'unexclude': case 'unblock': case 'unskip': {
                    ...
                    if (autoViewManager.includeContact(num)) {
                        const clean = num.replace(/[^0-9]/g, '');
                        await reply(`*Removed from skip list*\n\n ${clean} will now be auto-viewed again.`);
                    } else {
                ...
                case 'stats': case 'statistics': case 'info': {
                    ...
                    text += `Total Views : *${s.totalViewed}*\n`;
                    text += `Consecutive : ${s.consecutiveViews}\n`;
                    text += `Excluded : ${s.excludedCount}\n`;
                    text += `Queue : ${autoViewManager.queue.length} pending\n`;
                    if (s.lastViewed) {
