...
addCmd({
    ...
    handler: async (ctx) => {
        ...
        menu += `\n┏▣ HOW TO USE\n│ Reply with a number *1–${numbered.length}* to open a category\n│ Example: type *1* for ${numbered[0]?.label || 'first category'}\n┗▣`;
...
async function sendCategoryDetail(ctx, cat, allCmds) {
    ...
    out += `┗▣\\n`;
    out += `\\n┏▣ HOW TO USE\\n│ Reply *1–${cmds.length}* for command details\\n│ Example: type *1* for ${config.BOT_PREFIX}${cmds[0]?.name}\\n┗▣`;
...
addTrigger({
    ...
    handler: async (ctx) => {
        ...
        // Yield to sports.js if it has an active pending session for this chat
        try {
            const sportsMod = require('./sports');
            if (sportsMod && sportsMod.sportsPending) {
                const e = sportsMod.sportsPending.get(ctx.from);
                if (e && e.expiresAt > Date.now()) return;
            }
        } catch { /* sports not loaded */ }
        ...
        if (pendCmd) {
            ...
            if (idx < 0 || idx >= pendCmd.cmds.length) {
                return ctx.reply(`⚠️ *Invalid number: ${num}*\n\n*${pendCmd.catLabel}* only has *${pendCmd.cmds.length} commands* (1–${pendCmd.cmds.length}).\n\nType *.menu* to go back to categories.`);
            }
        ...
        if (idx < 0 || idx >= pending.categories.length) {
            return ctx.reply(`⚠️ *Invalid number: ${num}*\n\nThe menu only has *${pending.categories.length} categories* (1–${pending.categories.length}).\n\nType *.menu* to see the full list again.`);
        }
